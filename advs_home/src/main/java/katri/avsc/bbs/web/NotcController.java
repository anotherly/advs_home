...
public class NotcController {
	...
	private static final Log LOG = LogFactory.getLog(NotcController.class.getName());
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/bbs/notice/Board_Notc_List.do")
	public String selectNotcList(@RequestParam Map<String, String> paramMap, ModelMap model, HttpServletRequest request, HttpSession session) throws Exception {
		...
		paramMap.put("blbd_div_cd", "101");	//공지사항
		...
		return "board/Board_Notc_List.tiles";
	}
	 ...
	 * @param paramMap - 조회할 정보가 담긴 Map
	 * @param model
	 * @return "Board_Notc_Info"
	 * @exception Exception
	 */
	@RequestMapping(value = "/bbs/notice/Board_Notc_Info.do")
	public String notcInfo(@RequestParam Map<String, String> paramMap, ModelMap model, HttpServletRequest request) throws Exception {
		RequestURIModel uriModel = Util.getRequestURIModel(request.getRequestURI().replaceAll( request.getContextPath(), "")); //메뉴를 구성하기위한 설정
		LOG.debug(" uriModel["+uriModel.toString()+"]");
		model.addAttribute("uriModel", uriModel);

		LOG.debug(" ########## Board_Notc_Info.do ###########");
		...
		try{
			...
			e.printStackTrace();
			LOG.info("[DB Exception] 공지사항 접속수 증가  실패 : ");
		}

		Map<String, String> notcInfo = trndService.selectTrndInfo(paramMap);
		model.addAttribute("notcInfo", notcInfo);

		return "board/Board_Notc_Info.tiles";
	}
