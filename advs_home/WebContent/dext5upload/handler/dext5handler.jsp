...
/* 			System.out.println("newFileLocation : " + newFileLocation);
			System.out.println("responseFileServerPath : " + responseFileServerPath);
			System.out.println("responseFileName : " + responseFileName); */
			
			int cut1 = StringUtils.ordinalIndexOf(newFileLocation, "/", 2);
			int cut2 = StringUtils.lastIndexOf(newFileLocation, responseFileName);
			//System.out.println("#cut1 : " + cut1 + " cut2 : " + cut2);
			String path = newFileLocation.substring(cut1);
			//System.out.println("#path : " + path);
			
			parameterEntity.setNewFileLocation(newFileLocation); //Change NewFileLocation Value
			parameterEntity.setResponseFileServerPath(path); //Change ResponseFileServerPath Value
			parameterEntity.setResponseFileName(responseFileName); //Change ResponseFileName Value
	...
	*/
	...
	event.addOpenDownloadPathChangeEventListenerEx(new Raonwiz.Dext5.OpenDownloadPathChangeEventListenerEx() {
		public void OpenDownloadPathChangeEventEx(Raonwiz.Dext5.Process.Entity.UploadEventEntity parameterEntity) {
			...
			//System.out.println("downloadFilePath : " + downloadFilePath[0]);
			//System.out.println("downloadCustomValue : " + downloadCustomValue[0]);
			
			String saveRootFolder = "/Share2"; //운영(공유폴더로 붙으면 경로를 찾을 수 없으므로 직접적인 경로로 접근)
			//String saveRootFolder = "/home/app/avds_web/upload"; //운영
			//String saveRootFolder = "D:/dext5upload/dext5uploaddata"; //개발
			
			if(downloadFilePath != null && downloadFilePath.length > 0){
				for(int i=0; i<downloadFilePath.length; i++){
					downloadFilePath[i] = saveRootFolder + downloadFilePath[i];
				}
			}
			
			//System.out.println("set downloadFilePath : " + downloadFilePath[0]);
	...
	*/
	...
	String saveRootFolder = "/Share1/crf/"; //운영(공유폴더로 붙으면 경로를 찾을 수 없으므로 직접적인 경로로 접근)
	//String saveRootFolder = "/home/app/avds_web/upload"; //운영
	//String saveRootFolder = "D:\\dext5upload\\dext5uploaddata"; //개발
