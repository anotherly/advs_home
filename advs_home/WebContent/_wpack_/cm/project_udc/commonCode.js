/*amd /cm/project_udc/commonCode.xml 7608 1c2f322eb73acc4b470ffba30a77c21039b66bb1125d7c44aaa20222b4dbf0d8 */
...
scwin.setCommonCode = function( _$p , codeOptions, callbackFunc) {
	...
	for (i = 0; i < codeOptionsLen; i++) {
		...
		try {
			...
			if (typeof scwin.commonCodeList[dltId] === "undefined") {
				...
				dataListObj.setJSON($c.util.getJSON(scwin.commonCodeList[dltId]));
			}
	...
	var searchCodeGrpOption = {
		...
		target : "data:json," + $c.common.strSerialize(dltIdArr),
		isProcessMsg : false
	};
	...
	searchCodeGrpOption.submitDoneHandler = function(e) {
		for (codeGrpDataListId in e.responseJSON) {
			if (codeGrpDataListId.indexOf(scwin.DATA_PREFIX) > -1) {
				scwin.commonCodeList[codeGrpDataListId] = $c.common.strSerialize(e.responseJSON[codeGrpDataListId]);
			}
...
scwin.clearCodeDataListInfo = function(){
	scwin.commonCodeList = [];
};
