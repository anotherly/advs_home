                                ...
                                <div class="layout-content__view">
                                    <div class="noticeView-wrap">
                                        <div class="noticeView__top">
                                            <h5 class="noticeView__tit">${notcInfo.bdwrTtlNm}</h5>
                                            <dl class="noticeView__capt">
                                                <dt>작성일</dt>
                                                <dd>${notcInfo.regDateView}</dd>
                                                <dt>작성자</dt>
                                                <dd>${notcInfo.userNm}</dd>
                                            </dl>
                                        </div>
                                        <c:choose>
            								<c:when test="${notcInfo.attachFilename ne null}">
		                                        <div class="noticeView__crack">
		                                            <p class="crack__tit">첨부파일</p>
		                                            <div class="crack__links">
		                                                <a href="javascript:void(0);" title="${notcInfo.attachFilename} 첨부파일입니다" onclick="fn_Download('${notcInfo.attachFilename}')">${notcInfo.attachFilename}    (${notcInfo.fileSizeView} Bytes)</a>
		                                            </div>
		                                        </div>
                                            </c:when>
								            <c:otherwise>
                                        ...
                                        <div class="noticeView__link">
                                            <div class="link-next">
												...
												<c:choose>
													<c:when test="${notcInfo.nextBdwrSeq ne null}">
														<a href="javascript:detail_View('${notcInfo.blbdDivCd}','${notcInfo.nextBdwrSeq}');">${notcInfo.nextBdwrTtlNm}</a>
													</c:when>
													<c:otherwise>
                                            ...
                                            <div class="link-prev">
												...
												<c:choose>
													<c:when test="${notcInfo.prevBdwrSeq ne null}">
														<a href="javascript:detail_View('${notcInfo.blbdDivCd}','${notcInfo.prevBdwrSeq}');">${notcInfo.prevBdwrTtlNm}</a>
													</c:when>
													<c:otherwise>
