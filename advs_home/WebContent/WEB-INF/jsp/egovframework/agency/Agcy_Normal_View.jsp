...
<script type="text/javascript">
	...
	function addAllFiles(uploadID){
		...
		<c:forEach var="list" items="${mxFileList}" varStatus="status">
			...
			//RAONKUPLOAD.AddUploadedFile('${list.bSeq}', '${list.mxfile}', 'C:\\Share2\\SS\\FA\\00F\\카니발\\20230404\\${list.mxfile}', '${list.fileSize}', 'Custom Value', uploadID);
			if(i == 0){
    ...
    function RAONKUPLOAD_DownloadCompleteFile(uploadID, paramObj) {
  		$.ajax({
		    ...
		    url : "/open/normal/Open_Normal_FileInfo.do",
		    data : {"fileSize": paramObj.size, "bSeq": '${normalInfo.bSeq}'},
  ...
  $(document).ready(function() {
	...
	$('#offLine').on('click', function(){
		...
		c_form.bTitle.value = '${normalInfo.bTitle}';
		c_form.dataType.value = '${normalInfo.dataType}';
		c_form.dataTypeView.value = '${normalInfo.dataTypeView}';
  ...
  /* 수정 화면 function */
	function detail_View(bSeq, bbs) {
		...
		c_form.action = "<c:url value='/open/normal/Open_Normal_Info.do'/>";
		c_form.submit();
	}
...
<input type="hidden" name="file_nm" id="file_nm" value="${normalInfo.mxfile}"/>
									...
									<table class="table-basic table-basic--row">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													${normalInfo.dataTypeView}
												</td>
												<th scope="row">수집일자</th>
												<td>
													${normalInfo.collectDay}
												</td>
											</tr>
											<tr>
												<th scope="row">제목</th>
...
<%-- 									<button type="button" class="btn btn-width--l btn-color--gray" onclick="location.href='javascript:detail_View(${normalInfo.bSeq}, ${normalInfo.bbsSeq});' ">수정</button> --%>
