...
<script type="text/javaScript" defer>
  ...
  $(document).ready(function() {
    ...
    if( searchBseq == "" && searchScenarioCode1 == "" && searchScenarioCode2 == "" && searchScenarioCode3 == "" && searchRoad == "" && searchBdwr == "" && trim(c_form.searchWord.value).length == 0 ) {
      ...
      c_form.action = "<c:url value='/open/edge/Open_Edge_List.do'/>";
      c_form.submit();
    }
	}
...
//    c_form.dir_nm.value = "cits";
  ...
  /* 상세 화면 function */
	function detail_View(bSeq, bbs) {
		...
		c_form.action = "<c:url value='/open/edge/Open_Edge_View.do'/>";
		c_form.submit();
	}
	...
	function fn_Add_View(bbs) {
	    c_form.bbs_seq.value = bbs;
	    c_form.action = "<c:url value='/open/edge/Open_Edge_Inst.do'/>";
	    c_form.submit();
	}
  //checkbox 값 설정
	...
	<div class="container__inner">
	    <div class="content">
	        <div class="content__inner">
		        ...
		        <select id="searchScenarioCode1" name="searchScenarioCode1" class="w140px" title="검색대상 선택" onchange="form_Submit()">
		          <c:choose>
		        ...
		        <select id="searchScenarioCode2" name="searchScenarioCode2" class="w150px" title="검색대상 선택" style="width:150px" onchange="form_Submit()">
		          <c:choose>
		            <c:when test="${subCodeList eq null || empty subCodeList}">
		              <option value="">상황</option>
		            </c:when>
		            <c:otherwise>
		              <option value="">상황</option>
		              <c:forEach var="list" items="${subCodeList}" varStatus="loop">
		                <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq searchScenarioCode2}">selected</c:if>>${list.codeDetlNm}</option>
		              </c:forEach>
		            </c:otherwise>
		          </c:choose>
		        </select>
		        <select id="searchScenarioCode3" name="searchScenarioCode3" class="w150px" title="검색대상 선택" style="width:170px" onchange="form_Submit()">
		          <c:choose>
		            <c:when test="${subCodeList2 eq null || empty subCodeList2}">
		              <option value="">세부상황</option>
		            </c:when>
		            <c:otherwise>
		              <option value="">세부상황</option>
		              <c:forEach var="list" items="${subCodeList2}" varStatus="loop">
