															...
															<label for="">
																<select name="selectScenarioCode2" id="selectScenarioCode2" title="엣지케이스(도로/상황) 선택">
																	<c:choose>
																		...
																		<c:set var="code2" value="${list.codeDetlCd}" />
																		<option value="${code2}" <c:if test="${code2 eq searchAes}">selected</c:if>>${list.codeDetlNm}</option>
																		</c:forEach>
																		</c:otherwise>
																	</c:choose>
																</select>
...
<!--                                  <div class="table-wrap"> -->
								...
								<div class="btn-wrap btn-row justify-center mt-30 mb-80">
									<button type="button" class="btn btn-width--l btn-color--white" onclick="location.href='/open/normal/Open_Normal_List.do?bbs_seq=2080' ">취소</button>
									<button type="button" class="btn btn-width--l btn-color--navy" onclick="location.href='javascript:fn_Insert(2080);' ">등록</button>
								</div>
