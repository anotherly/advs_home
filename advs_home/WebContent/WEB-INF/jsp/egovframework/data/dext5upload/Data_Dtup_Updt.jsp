...
<script type="text/javascript" src="/dext5upload/js/dext5upload.js"></script>
    <script type="text/javascript">
        ...
        function addAllFiles(uploadID){
            ...
            $("#fileName").val("${dtupInfo.saveNm}");
            $("#fileSize").val("${dtupInfo.fileSize}");
            $("#filePath").val("/dtup/${dtupInfo.saveNm}");        	
  	    }
...
//		if(confirm("수정 하시겠습니까?")) {
      ...
      // 업로드 파일 선택 여부 확인
/*       var updata = $('#a2mdms').data('dms').getData();
      if(updata.length > 0 ) {
        //업로드 컴포넌트 호출
        submit1();
      } else {
        c_form.file_up.value = "N";
        fn_Save();
      } */
//		}
    //기존업로드 컴포넌티 제거로 인하여 fn_Save 호출
  ...
  /* 등록 function */
	function fn_Save() {
		document.listForm.action = "<c:url value='/data/upload/Data_Dtup_Updt_Process.do'/>";
...
<input type="hidden" name="bbs_seq_old" id="bbs_seq_old" value="${bbs_seq}" />
<input type="hidden" name="bdwr_seq" id="bdwr_seq" value="${dtupInfo.bdwrSeq}" />
        ...
        <table summary="" class="">
          ...
          <tbody>
                      ...
                      <c:choose>
                        ...
                        <c:otherwise>
                          ...
                          <c:forEach var="list" items="${bbs_list}" varStatus="loop">
                            <option value="${list.bbsSeq}" <c:if test="${list.bbsSeq eq bbs_seq}">selected</c:if>>${list.bbsNm}</option>
                          </c:forEach>
                        </c:otherwise>
                      </c:choose>
                    </select>
            ...
            <tr>
              ...
              <td colspan="3">
                <input type="text" class="w100p" id="b_title" name="b_title" value="${dtupInfo.bTitle}" onkeyup="checkLength(this,255)">
              </td>
            </tr>
            <tr>
              <th class="border_l_none"><b class="font_red">*</b> 내용</th>
              <td colspan="3">
                <textarea title="내용 입력" style="width:100%; height:300px;background-color:#fff; display:none;" name="b_content" id="b_content" title="">${dtupInfo.bContent}</textarea>
              </td>
            </tr>
            <!--
            <tr>
              ...
              <td>
                <input type="text" readonly="" class="w30p" id="b_from_day" name="b_from_day" value="${dtupInfo.bFromDayView}"> ~
                <input type="text" readonly="" class="w30p" id="b_to_day" name="b_to_day" value="${dtupInfo.bToDayView}">
              </td>
              <th class="border_l_none">비밀번호</th>
              <td>
                <input type="password" class="w50p" id="b_password" name="b_password" value="${dtupInfo.bPassword}" onkeyup="checkLength(this,20)">
                <i class="checkbox"><input type="checkbox" id="chk_i"  name="chk_i" onclick="fn_check_box(this)" <c:if test="${dtupInfo.bImportantYn eq 'Y'}">checked</c:if>><label for="chk_i">중요</label></i>
                <input type="hidden" name="b_important_yn" id="b_important_yn" value="${dtupInfo.bImportantYn}" />
              </td>
            </tr>
            -->
            <tr>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      <c:forEach var="list" items="${code_driving_mode }" varStatus="loop">
                        <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq dtupInfo.drivingMode}">selected</c:if>>${list.codeDetlNm}</option>
                      </c:forEach>
                    </c:otherwise>
                  </c:choose>
                </select>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      <c:forEach var="list" items="${code_weather }" varStatus="loop">
                        <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq dtupInfo.weather}">selected</c:if>>${list.codeDetlNm}</option>
                      </c:forEach>
                    </c:otherwise>
                  </c:choose>
                </select>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      <c:forEach var="list" items="${code_road_situation }" varStatus="loop">
                        <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq dtupInfo.roadSituation}">selected</c:if>>${list.codeDetlNm}</option>
                      </c:forEach>
                    </c:otherwise>
                  </c:choose>
                </select>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      <c:forEach var="list" items="${code_road_type_cd }" varStatus="loop">
                        <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq dtupInfo.roadTypeCd}">selected</c:if>>${list.codeDetlNm}</option>
                      </c:forEach>
                    </c:otherwise>
                  </c:choose>
                </select>
            ...
            <tr>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      <c:forEach var="list" items="${code_act_info_type }" varStatus="loop">
                        <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq dtupInfo.actInfoType}">selected</c:if>>${list.codeDetlNm}</option>
                      </c:forEach>
                    </c:otherwise>
                  </c:choose>
                </select>
              ...
              <td>
                <input type="text" class="w100p" id="etc_info" name="etc_info" value="${dtupInfo.etcInfo}" onkeyup="checkLength(this,255)">
              </td>
            </tr>
            <tr>
	                  ...
	                  <c:choose>
	                    ...
	                    <c:otherwise>
	                      <c:forEach var="list" items="${mydata_attach_cd}" varStatus="loop">
	                        <option value="${list.codeDetlCd}"<c:if test="${list.codeDetlCd eq dtupInfo.fileType }"> selected="selected"</c:if>>${list.codeDetlNm}</option>
	                      </c:forEach>
	                    </c:otherwise>
	                  </c:choose>
	                </select>            	
            ...
            <tr>
              <th rowspan="2" class="border_l_none">첨부파일</th>
              <td colspan="3" class="left border_l_none lh_sm">
			    <script type="text/javascript">
              ...
              <%-- 
                <c:choose>
					<c:when test="${dtupInfo.saveNm ne null}">
						<a href="javascript:void(0);" title="${dtupInfo.saveNm} 첨부파일입니다" onclick="fn_Download('${dtupInfo.saveNm}','dtup')"><img src="/images/avsc/ico_file.png" alt="다운로드" />&nbsp<i class="font_lGray">${dtupInfo.saveNm}</i></a>
					</c:when>
					<c:otherwise>
						-
					</c:otherwise>
            ...
            <tr>
              <td colspan="3" class="left border_l_none lh_sm"><i class="font_orange">※ 파일을 선택하시면 기존 등록된 파일이 삭제됩니다</i></td>
            </tr>
            -->
          </tbody>
        </table>
...
<!--       <div id="file_upload">
        ...
        <table summary="" class="">
          ...
          <tbody>
            <tr>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      ...
                      <c:forEach var="list" items="${car_list}" varStatus="loop">
                        <option value="${list.vehicleId}|${list.carType}|${list.carModel}|${list.movieSensorModel}|${list.lidarModel}|${list.radarModel}|${list.autocarLevel}" <c:if test="${list.vehicleId eq dtupInfo.vehicleId}">selected</c:if>>${list.vehicleId}</option>
                      </c:forEach>
                      <option value="new">---신규등록---</option>
                    </c:otherwise>
                  </c:choose>
              ...
              <td>
                <input type="text" class="w70p" id="vehicle_id" name="vehicle_id" value="${dtupInfo.vehicleId}" readonly onkeyup="checkLength(this,20)">
            ...
            <tr>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      <c:forEach var="list" items="${code_car_type}" varStatus="loop">
                        <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq dtupInfo.carType}">selected</c:if>>${list.codeDetlNm}</option>
                      </c:forEach>
                    </c:otherwise>
                  </c:choose>
                </select>
              ...
              <td>
                <input type="text" class="w100p" id="car_model" name="car_model" value="${dtupInfo.carModel}" onkeyup="checkLength(this,255)">
              </td>
            </tr>
            <tr>
              <th class="border_l_none">영상센서모델</th>
              <td>
                <input type="text" class="w100p" id="movie_sensor_model" name="movie_sensor_model" value="${dtupInfo.movieSensorModel}" onkeyup="checkLength(this,255)">
              </td>
              <th class="border_l_none">라이다모델</th>
              <td>
                <input type="text" class="w100p" id="lidar_model" name="lidar_model" value="${dtupInfo.lidarModel}" onkeyup="checkLength(this,255)">
              </td>
            </tr>
            <tr>
              <th class="border_l_none">레이다모델</th>
              <td>
                <input type="text" class="w100p" id="radar_model" name="radar_model" value="${dtupInfo.radarModel}" onkeyup="checkLength(this,255)">
              </td>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      ...
                      <c:forEach var="list" items="${code_car_level}" varStatus="loop">
                        <option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq dtupInfo.autocarLevel}">selected</c:if>>${list.codeDetlNm}</option>
                      </c:forEach>
                    </c:otherwise>
                  </c:choose>
                </select>
      ...
      <div class="table_row mt40 mb10"<c:if test="${dataset_list eq null || empty dataset_list}"> style="display:none;"</c:if>>
        <p class="contTit1">데이터 셋 정보</p>
        <p class="right"><i class="font_sky">※ 자료구성 및 자료에 대한 세부정보를 기입해주세요</i></p>
      </div>

      <div class="table2"<c:if test="${dataset_list eq null || empty dataset_list}"> style="display:none;"</c:if>>
        <table summary="" class="">
          <colgroup>
            <col width="15%">
            <col width="">
          ...
          <tbody>
                  ...
                  <c:choose>
                    ...
                    <c:otherwise>
                      ...
                      <c:forEach var="list" items="${dataset_list}" varStatus="loop">
                        <option value="${list.bdwrSeq}|${list.bbsSeq}" <c:if test="${list.bdwrSeq eq dtupInfo.bdwrSeq}">selected</c:if>>${list.bdwrTtlNm}</option>
                      </c:forEach>
                      <!-- <option value="new">---신규등록---</option> -->
                    </c:otherwise>
                  </c:choose>
            ...
            <tr>
              <th class="border_l_none"><b class="font_red">*</b> 제목</th>
              <td>
                <input type="text" class="w100p" id="bdwr_ttl_nm" name="bdwr_ttl_nm" value="${dtupInfo.bdwrTtlNm}" onkeyup="checkLength(this,255)">
              </td>
            </tr>
            <tr>
              <th class="border_l_none"><b class="font_red">*</b> 내용</th>
              <td>
                <textarea title="내용 입력" style="width:100%; height:300px;background-color:#fff; display:none;" name="bdwr_cts" id="bdwr_cts" title="">${dtupInfo.bdwrCts}</textarea>
              </td>
            </tr>
            <!--
            <tr>
              ...
              <td>
                <input type="text" class="w100p" id="data_form" name="data_form" value="${dtupInfo.dataForm}" onkeyup="checkLength(this,100)">
              </td>
            </tr>
            -->
            <tr>
      ...
      <div class="center mt20">
        <input type="button" class="btn_sky btn_lg mr5" onclick="javascript:fn_Save_Check();" value="저장">
        <a href="/data/upload/Data_Dtup_Info.do?b_seq=${b_seq}&bbs_seq=${bbs_seq}" class="btn_gray btn_lg">취소</a>
      </div>
    </div>
    <!-- //contents -->
  </div>
