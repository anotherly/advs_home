  ...
  $(document).ready(function() {
	...
	function fn_Update(bseq, bbs) {
	    if(confirm("등록된 정보를 수정 하시겠습니까?")) {
	      ...
	      c_form.action = "<c:url value='/open/v2x/Open_V2x_Updt_Process.do'/>";
	      c_form.submit();
	    }
	}
			...
			<div class="content__inner">
									...
									<table class="table-basic table-basic--row table--myUpload">
										...
										<tbody>
															...
															<label for="">
																<select name="selectDataType" id="selectDataType" title="수집차량 모델 선택">
																	<c:choose>
																		...
																		<option value="${list.bbsSeq}" <c:if test="${list.bbsSeq eq v2xInfo.dataType}">selected</c:if>>${list.bbsNm}</option>
																		</c:forEach>
																		</c:otherwise>
																	</c:choose>
																</select>
												...
												<td>
													<div class="ui-form">
														<div class="ui-form-block">
															<input type="text" class="ui-input ui-form-width--l" name="b_title" id="b_title" Value="${v2xInfo.bTitle}">
														</div>
													</div>
												</td>
											</tr>
											<tr>
												<th scope="row">내용</th>
												<td>
													<textarea title="내용 입력" style="width:100%; height:300px;background-color:#fff; display:none;" name="b_content" id="b_content" title="">${v2xInfo.bContent}</textarea>
												</td>
											</tr>
											<tr>
														...
														<select name="selectDrivingMode" id="selectDrivingMode" title="주행모드 선택">
															<c:choose>
																...
																<c:forEach var="list" items="${code_driving_mode}" varStatus="loop">
																	<option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq v2xInfo.drivingMode}">selected</c:if>>${list.codeDetlNm}</option>
																</c:forEach>
																</c:otherwise>
															</c:choose>
														</select>
				...
				<!-- 										<div id="raonkuploader_holder_kupload" style="width:auto; height:250px"><iframe id="raonkuploader_frame_kupload" title="RAONKUpload kupload" frameborder="0" src="" style="width: 100%; height: 100%; border-width: 0px;"></iframe></div> -->
								...
								<div class="btn-wrap btn-row justify-center mt-30 mb-80">
									...
									<button type="button" class="btn btn-width--l btn-color--gray" onclick="location.href='javascript:fn_Update(${v2xInfo.bSeq}, ${v2xInfo.bbsSeq});' ">저장</button>
