...
<script type="text/javascript">
	...
	var date = new Date();
	var curDate = date.getFullYear() + ("0" + (1 + date.getMonth())).slice(-2)
			...
			function() {
				...
				$('#selectDataType').change(
						function() {
							...
							realPath = realPath1 + realPath2 + realPath3
									+ code4 + "/" + curDate.toString();

							RAONKUPLOAD.SetConfig('FolderNameRule', realPath,
									G_UploadID);
						});
			});
	...
	function RAONKUPLOAD_UploadComplete(uploadID) {
		...
		if (jsonAll != null) {
			...
			for (var i = 0; i < jsonWebFileLength; i++) {
				...
				RAONKUPLOAD.AddUploadedFile(jsonWebFile.order[i],
						jsonWebFile.originalName[i], jsonWebFile.uploadPath[i],
						jsonWebFile.size[i], '', uploadID);
			}
		}
		...
		if (jsonNew != null) {
			...
			for (var i = 0; i < jsonNewLength; i++) {
				...
				RAONKUPLOAD.AddUploadedFile(jsonNew.order[i],
						jsonNew.originalName[i], jsonNew.uploadPath[i],
						jsonNew.size[i], '', uploadID);
			}
		}
	...
	function fn_Insert(bbs) {
		if (confirm("등록 하시겠습니까?")) {
			...
			document.listForm.action = "<c:url value='/open/normal/Open_Normal_Inst_Process.do'/>";
							...
							<div class="lnb">
								...
								<div class="lnb-list">
									<div class="lnb-item is-active">
										<a href="/open/normal/Open_Normal_List.do?bbs_seq=3010">일반시나리오
											데이터셋</a>
									</div>
									<div class="lnb-item">
										<a href="/open/edge/Open_Edge_List.do?bbs_seq=3020">엣지케이스
											시나리오<br />데이터셋
										</a>
									</div>
									<div class="lnb-item">
										<a href="/open/v2x/Open_V2x_List.do?bbs_seq=3030">V2X 데이터셋</a>
									</div>
								</div>
							</div>
								...
								<div class="layout-content__inner">
										...
										<table class="table-basic table-basic--row table--myUpload">
											...
											<tbody>
												<tr>
													...
													<td colspan="3">
														<div class="ui-form">
															<label for=""> <select name="selectScenarioCode1"
																id="selectScenarioCode1" title="일반시나리오(시간) 선택">
																	<c:choose>
																		<c:when
																			test="${scenario_code1 eq null || empty scenario_code1}">
																			<option value="">시간</option>
																		</c:when>
																		<c:otherwise>
																			<option value="">시간</option>
																			<c:forEach var="list" items="${scenario_code1}"
																				varStatus="loop">
																				<option value="${list.codeDetlCd}"
																					<c:if test="${list.codeDetlCd eq searchAgs}">selected</c:if>>${list.codeDetlNm}</option>
																			</c:forEach>
																		</c:otherwise>
																	</c:choose>
															</select>
															</label> <label for=""> <select
																name="selectScenarioCode2" id="selectScenarioCode2"
																title="엣지케이스(도로/상황) 선택">
																	<c:choose>
																		<c:when
																			test="${scenario_code2 eq null || empty scenario_code2}">
																			<option value="">도로/상황</option>
																		</c:when>
																		<c:otherwise>
																			<option value="">도로/상황</option>
																			<c:forEach var="list" items="${scenario_code2}"
																				varStatus="loop">
																				<option value="${list.codeDetlCd}"
																					<c:if test="${list.codeDetlCd eq searchAes}">selected</c:if>>${list.codeDetlNm}</option>
																			</c:forEach>
																		</c:otherwise>
																	</c:choose>
															</select>
															</label> <label for=""> <select
																name="selectScenarioCode3" id="selectScenarioCode3"
																title="특수시나리오(날씨) 선택">
																	<c:choose>
																		<c:when
																			test="${scenario_code3 eq null || empty scenario_code3}">
																			<option value="">날씨</option>
																		</c:when>
																		<c:otherwise>
																			<option value="">날씨</option>
																			<c:forEach var="list" items="${scenario_code3}"
																				varStatus="loop">
																				<option value="${list.codeDetlCd}"
																					<c:if test="${list.codeDetlCd eq searchAss}">selected</c:if>>${list.codeDetlNm}</option>
																			</c:forEach>
																		</c:otherwise>
																	</c:choose>
															</select>
												...
												<tr>
													<th scope="row">수집차량모델</th>
													<td>
														<div class="ui-form">
															<label for=""> <select name="selectDataType"
																id="selectDataType" title="수집차량모델 선택">
																	<c:choose>
																		<c:when test="${bbs_list eq null || empty bbs_list}">
																			<option value="">수집차량모델</option>
																		</c:when>
																		<c:otherwise>
																			<option value="">수집차량모델</option>
																			<c:forEach var="list" items="${bbs_list}"
																				varStatus="loop">
																				<option value="${list.bbsSeq}"
																					<c:if test="${list.bbsSeq eq searchBseq}">selected</c:if>>${list.bbsNm}</option>
																			</c:forEach>
																		</c:otherwise>
																	</c:choose>
															</select>
														...
														<div
															...
															<div class="ui-form-block">
																<input type="text"
																	...
																	title="수집일자" id=collect_day name="collect_day">
															</div>
														</div>
													</td>
												</tr>
												<tr>
													...
													<td colspan="3">
														<div class="ui-form">
															<div class="ui-form-block">
																<input type="text" class="ui-input ui-form-width--l"
												...
												<tr>
													<th scope="row">내용</th>
													<td colspan="3"><textarea title="내용 입력"
															style="width: 100%; height: 300px; background-color: #fff; display: none;"
															name="b_content" id="b_content" title=""></textarea></td>
												</tr>
												<tr>
													<th scope="row">주행모드</th>
													<td colspan="3">
														<div class="ui-form">
															<select name="selectDrivingMode" id="selectDrivingMode"
																title="주행모드 선택">
																<c:choose>
																	<c:when
																		test="${code_driving_mode eq null || empty code_driving_mode}">
																		<option value="">주행모드</option>
																	</c:when>
																	<c:otherwise>
																		<option value="">주행모드</option>
																		<c:forEach var="list" items="${code_driving_mode}"
																			varStatus="loop">
																			<option value="${list.codeDetlCd}"
																				<c:if test="${list.codeDetlCd eq searchAss}">selected</c:if>>${list.codeDetlNm}</option>
																		</c:forEach>
									...
									<div class="btn-wrap btn-row justify-center mt-30 mb-80">
										<button type="button"
											...
											onclick="location.href='/open/normal/Open_Normal_List.do?bbs_seq=3010' ">취소</button>
										<button type="button" class="btn btn-width--l btn-color--navy"
											onclick="location.href='javascript:fn_Insert(3010);' ">등록</button>
									</div>
