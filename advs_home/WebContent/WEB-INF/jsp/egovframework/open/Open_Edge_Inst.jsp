	...
	function fn_Insert(bbs) {
	  ...
	  if(RAONKUPLOAD.GetTotalFileCount(G_UploadID, "total")==0){
			...
			return;
 			...
 			document.listForm.action = "<c:url value='/open/edge/Open_Edge_Inst_Process.do'/>";
					...
					<div class="layout-container__inner layout-container--row">
						<div class="lnb">
							...
							<div class="lnb-list">
	 							...
	 							<div class="lnb-item is-active"><a href="/open/edge/Open_Edge_List.do?bbs_seq=3020">엣지케이스 시나리오<br/>데이터셋</a></div>
	                            <div class="lnb-item"><a href="/open/v2x/Open_V2x_List.do?bbs_seq=3030">V2X 데이터셋</a></div>
							</div>
						</div>
															...
															<label for="">
																<select name="selectScenarioCode1" id="selectScenarioCode1" title="엣지케이스 종류 선택" onchange="fn_EdgeCode()">
																	<c:choose>
																		...
																		<option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq selectScenarioCode1}">selected</c:if>>${list.codeDetlNm}</option>
																		</c:forEach>
																		</c:otherwise>
																	</c:choose>
																</select>
...
<!--                                  </div> -->
								...
								<div class="btn-wrap btn-row justify-center mt-30 mb-80">
									<button type="button" class="btn btn-width--l btn-color--white" onclick="location.href='/open/edge/Open_Edge_List.do?bbs_seq=3020' ">취소</button>
									<button type="button" class="btn btn-width--l btn-color--navy" onclick="location.href='javascript:fn_Insert(3020);' ">등록</button>
								</div>
