...
//					c_form.b_content.value="";
	      ...
	      c_form.action = "<c:url value='/open/edge/Open_Edge_Updt_Process.do'/>";
	      c_form.submit();
	    }
	}
  ...
  /* 코드 조회 ajax  */
	function fn_EdgeCode(){
	  	var refVal2=  $("#selectScenarioCode1").val();
	  	var addData; 
	    $.ajax({
			type : "POST",
			dataType: "json",
...
// 				console.log(data);
				addData = data;
				$("#selectScenarioCode2").children('option:not(:first)').remove();
	     		var laborOption = "";
				for(i=0; i<addData.length; i++){
// 		 		  alert(addData[i].codeDetlNm);
	 	        	var prgm_name = addData[i].codeDetlNm
				  	var prgm_idx = addData[i].codeDetlCd
				  
				 	laborOption = "<option value='" + prgm_idx + "'>" + prgm_name + "</option>";
				 $("#selectScenarioCode2").append(laborOption);
			 	}
// 	 			alert("정상조회");
			},
			error: function(data){
	...
	/* 코드 조회2 ajax  */
  	function fn_EdgeCode2(){
		var refVal2=  $("#selectScenarioCode2").val();
	 	var addData; 
		$.ajax({
			type : "POST",
			dataType: "json",
...
// 				console.log(data);
				addData = data;
				$("#selectScenarioCode3").children('option:not(:first)').remove();
	     		var laborOption = "";
				for(i=0; i<addData.length; i++){
	// 	 			alert(addData[i].codeDetlNm);
		 	        var prgm_name = addData[i].codeDetlNm
					var prgm_idx = addData[i].codeDetlCd
				 	...
				 	laborOption = "<option value='" + prgm_idx + "'>" + prgm_name + "</option>";
				 	$("#selectScenarioCode3").append(laborOption);
				 }
// 	 			alert("정상조회");
			},
			error: function(data){
...
<input type="hidden" name="file_nm" id="file_nm" value="${edgeInfo.saveNm}"/>
			...
			<div class="content__inner">
							...
							<div class="layout-content__inner">
								...
								<div class="table-wrap">
									<table class="table-basic table-basic--row table--myUpload">
										<caption>공공데이터 &gt; 엣지케이스 시나리오 데이터셋 테이블</caption>
										<colgroup>
											<col style="width:15%">
										...
										<tbody>
											<tr>
												...
												<td>
													<div class="ui-form">
															<label for="">
																<select name="selectScenarioCode1" id="selectScenarioCode1" title="엣지케이스 종류 선택" onchange="fn_EdgeCode()">
																	<c:choose>
															...
															<label for="">
																<select name="selectScenarioCode2" id="selectScenarioCode2" title="엣지케이스 상황 선택" onchange="fn_EdgeCode2()">
																	<c:choose>
															...
															<label for="">
																<select name="selectScenarioCode3" id="selectScenarioCode3" title="엣지케이스 세부상황 선택">
																	<c:choose>
															...
															<label for="">
																<select name="selectDataType" id="selectDataType" title="수집차량 모델 선택">
																	<c:choose>
																		...
																		<option value="">데이터유형</option>
																		</c:when>
																		<c:otherwise>
																		<option value="">데이터유형</option>
																		<c:forEach var="list" items="${bbs_list}" varStatus="loop">
																		<option value="${list.bbsSeq}" <c:if test="${list.bbsSeq eq edgeInfo.dataType}">selected</c:if>>${list.bbsNm}</option>
																		</c:forEach>
																		</c:otherwise>
																	</c:choose>
																</select>
												...
												<td>
													<div class="ui-form">
														<div class="ui-form-block">
															<input type="text" class="ui-input ui-form-width--l" name="b_title" id="b_title" Value="${edgeInfo.bTitle}">
														</div>
													</div>
												</td>
											</tr>
											<tr>
												<th scope="row">내용</th>
												<td>
													<textarea title="내용 입력" style="width:100%; height:300px;background-color:#fff; display:none;" name="b_content" id="b_content" title="">${edgeInfo.bContent}</textarea>
												</td>
											</tr>
											<tr>
														...
														<select name="selectDrivingMode" id="selectDrivingMode" title="주행모드 선택">
															<c:choose>
																...
																<c:forEach var="list" items="${code_driving_mode}" varStatus="loop">
																	<option value="${list.codeDetlCd}" <c:if test="${list.codeDetlCd eq edgeInfo.drivingMode}">selected</c:if>>${list.codeDetlNm}</option>
																</c:forEach>
																</c:otherwise>
															</c:choose>
														</select>
				...
				<!-- 										<div id="raonkuploader_holder_kupload" style="width:auto; height:250px"><iframe id="raonkuploader_frame_kupload" title="RAONKUpload kupload" frameborder="0" src="" style="width: 100%; height: 100%; border-width: 0px;"></iframe></div> -->
								...
								<div class="btn-wrap btn-row justify-center mt-30 mb-80">
									...
									<button type="button" class="btn btn-width--l btn-color--gray" onclick="location.href='javascript:fn_Update(${edgeInfo.bSeq}, ${edgeInfo.bbsSeq});' ">저장</button>
