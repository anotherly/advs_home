...
<!-- header -->
<headr id="top_nav" onmouseover="fnOpenGnb();" onmouseout="fnCloseGnb();">
	<div class="top_head">
		<a href="/main/Main.do"><h1 class="top_logo">자동차안전연구원 자율주행 데이111터 공유센터</h1></a>
		<div class="nav_gnb">
<%
		if(!login) {
%>
...
%>
				<li class="depth-2"><a href="javascript:<%=menuSub.get("scriptMethod") %>('<%=menuSub.get("menuObject") %>','<%=auth_id%>','<%=user_id%>');"><%=menuSub.get("menuNm") %></a></li>
<%					
					}else{
%>
				<li class="depth-2"><a href="<%=menuSub.get("menuObject") %>"><%=menuSub.get("menuNm") %></a></li>
<%
					}
					}
				}
