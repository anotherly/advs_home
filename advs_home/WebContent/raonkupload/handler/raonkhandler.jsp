...
<%@page import="com.raonwiz.kupload.RAONKHandler"%><%@page import="com.raonwiz.kupload.event.*"%><%@page import="com.raonwiz.kupload.util.EventVo"%><%@page import="com.raonwiz.kupload.common.ImageApi"%><%@ page contentType="text/html;charset=utf-8"%><%
	...
	event.addBeforeUploadEventListener(new BeforeUploadEventListener() {
		public void beforeUploadEvent(EventVo eventVo) {
	        ...
	        //eventVo.setResponseFileServerPath(path); //Change ResponseFileServerPath Value
	...
	*/
	...
	event.addAfterUploadEventListener(new AfterUploadEventListener() {
		public void afterUploadEvent(EventVo eventVo) {
			...
			int cut1 = StringUtils.ordinalIndexOf(strNewFileLocation, "/", 2);
			int cut2 = StringUtils.lastIndexOf(strNewFileLocation, strGuid);
			//System.out.println("#strNewFileLocation : " + strNewFileLocation);
			//System.out.println("#strGuid : " + strGuid);
			//System.out.println("#cut1 : " + cut1 + " cut2 : " + cut2);
			String path = strNewFileLocation.substring(cut1);
	...
	event.addBeforeDownloadEventListener(new BeforeDownloadEventListener() {
		public void beforeDownloadEvent(EventVo eventVo) {
			...
			if(aryDownloadFilePath != null && aryDownloadFilePath.length > 0){
				for(int i=0; i<aryDownloadFilePath.length; i++){
					aryDownloadFilePath[i] = saveRootFolder + aryDownloadFilePath[i];
	...
	*/
    ...
    String strRootFolder = "/Share2"; //운영(공유폴더로 붙으면 경로를 찾을 수 없으므로 직접적인 경로로 접근)
	...
	upload.settingVo.setPhysicalPath(strRootFolder);
	...
	String[] arrAllowUploadDirectoryPath = {strRootFolder};
	...
	String[] arrAllowDownloadDirectoryPath = {strRootFolder, request.getSession().getServletContext().getRealPath("/")};
